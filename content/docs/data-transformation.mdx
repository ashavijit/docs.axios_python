...
# A function to automatically convert payloads to protobufs
# or complex nested dicts into properly serialized JSON strings
def convert_to_json(data, headers):
    if isinstance(data, dict):
        # We also have access to the headers dict mapped to the request
        headers["Content-Type"] = "application/json"
        
        # We must return the transformed payload
        return json.dumps(data)
    
    # Always fallback to returning data as-is if no transform occurs
    return data

# Pass an array of transformer callables
api = create({
    "transform_request": [convert_to_json]
})

# Under the hood, this payload is stringified.
...
# A function mapping custom un-wrapping logic over all responses
def unwrap_envelope_and_decode(data):
    if isinstance(data, dict):
        payload = data.get("internal_payload", data)
        # Apply strict data manipulation
...
# Register the response pipeline hook
api = create({
