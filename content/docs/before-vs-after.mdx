...
# Every file that needs to hit Github needs to rebuild this
...
# Define configuration exactly once
...
# The engine handles all sleeping, logging, and error catching automatically
def fetch_flaky_endpoint():
...
def get_users():
    ...
    # Have to remember to unpack 'data' everywhere
    if "data" in payload:
        return payload["data"]
    return payload
```
...
# Attach the interceptor once
api.interceptors.response.use(unwrap_data)

# Everywhere else:
def get_users():
    # `response.data` is automatically unwrapped
...
class SDK:
    ...
    async def async_get_data(self):
        # Uses the exact same underlying logic, interceptors, and config
